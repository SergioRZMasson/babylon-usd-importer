...
const packageDir = resolve(projectRoot, 'package');
const wasmSourceDir = resolve(packageDir, 'dist', 'wasm');
if (!existsSync(wasmSourceDir)) {
    throw new Error(
        `WebAssembly artifacts not found at ${wasmSourceDir}.\n` +
            'Build the package first: npm --prefix ../package run build',
    );
}
...
export default {
    ...
    resolve: {
        ...
        alias: {
            ...
            'babylon-usd-importer': resolve(packageDir, 'dist', 'index.js'),
        },
    },
