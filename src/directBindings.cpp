...
public:
    ...
    void complete(babylon::usd_importer::SceneBuffers&& buffers,
                  double totalMs,
                  double stageOpenMs)
    {
        m_ok = true;
...
extract(const std::string& inputPath)
{
    ...
    babylon::usd_importer::SceneBuffers buffers;
    if (!babylon::usd_importer::buildSceneBuffers(stage, buffers)) {
...
void
setRootAssetPath(const std::string& path)
{
    WebResolver::SetRootAssetPath(path);
}
...
EMSCRIPTEN_BINDINGS(openusd_babylon_direct)
{
    ...
    emscripten::function("setRootAssetPath", &setRootAssetPath);
